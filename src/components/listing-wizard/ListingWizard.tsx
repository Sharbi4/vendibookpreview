  ...
  const performAutoSave = useCallback(async () => {
    ...
    try {
      ...
      const draftData = {
        ...
        price_monthly: formData.price_monthly && formData.price_monthly.trim() ? parseFloat(formData.price_monthly.replace(/[^0-9.]/g, '')) || null : null,
  ...
  const saveListing = async (publish: boolean) => {
    ...
    try {
      ...
      const listingData = {
        ...
        // Multi-slot capacity
        total_slots: formData.total_slots || 1,
        slot_names: formData.slot_names.length > 0 ? formData.slot_names : null,
      };
