...
export function PaymentsTransitionModal() {
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => { if (!v) void close(); }}>
      <DialogContent className="max-w-lg border-2 border-white/12 bg-[#08080a]/95 backdrop-blur-xl">
        <DialogHeader>
          <div className="w-10 h-10 rounded-full bg-primary/10 ring-1 ring-primary/25 flex items-center justify-center mb-2">
            <Banknote className="h-5 w-5 text-primary" />
          </div>
          <DialogTitle className="text-lg">
            Great news&mdash;Vendibook payments are now powered by PayPal.
          </DialogTitle>
