    ...
    <div className="min-h-screen bg-background relative">
      ...
      <div className="fixed inset-0 pointer-events-none overflow-hidden z-0">
        <motion.div
          ...
          className="absolute -top-32 -left-32 w-[500px] h-[500px] rounded-full bg-[hsl(14,100%,57%)]/8 blur-[120px]"
        />
        <motion.div
          animate={{ x: [0, -30, 40, 0], y: [0, 30, -30, 0], scale: [1, 0.9, 1.15, 1] }}
          transition={{ repeat: Infinity, duration: 25, ease: 'easeInOut' }}
          className="absolute top-1/3 right-0 w-[600px] h-[600px] rounded-full bg-[hsl(40,100%,49%)]/6 blur-[140px]"
        />
        <motion.div
          animate={{ x: [0, 20, -30, 0], y: [0, -20, 40, 0] }}
          transition={{ repeat: Infinity, duration: 18, ease: 'easeInOut' }}
          className="absolute bottom-0 left-1/4 w-[400px] h-[400px] rounded-full bg-[hsl(14,80%,50%)]/5 blur-[100px]"
        />
      </div>
      ...
      <div className="relative z-10 flex">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="sticky top-16 z-30">
            <div className="backdrop-blur-2xl bg-background/60 border-b border-border/30">
              <div className="max-w-6xl mx-auto px-4 py-3 flex flex-wrap items-center gap-2.5">
                {/* Search input */}
                <div className="relative flex-1 min-w-[180px] max-w-xs">
                  ...
                  <input
                    ...
                    className="w-full h-9 pl-9 pr-3 rounded-full bg-muted/40 backdrop-blur border border-border/30 text-sm focus:outline-none focus:ring-2 focus:ring-[hsl(14,100%,57%)]/30 transition-all"
                  />
                </div>
                ...
                <div className="min-w-[160px] max-w-[200px]">
                  <LocationSearchInput
                    ...
                    className="h-9 rounded-full bg-muted/40 backdrop-blur border-border/30 text-sm"
                  />
                </div>

                {/* Mode pills */}
                <div className="flex h-9 rounded-full overflow-hidden border border-border/30 backdrop-blur bg-muted/30">
                ...
                <Select value={sortBy} onValueChange={(v) => setSortBy(v as typeof sortBy)}>
                  <SelectTrigger className="h-9 w-36 text-xs rounded-full bg-muted/40 backdrop-blur border-border/30">
                ...
                <Button size="sm" onClick={handleSearch} className="h-9 rounded-full bg-foreground text-background hover:bg-foreground/90 px-5 text-xs font-semibold">
                  Search
                </Button>

                {/* Mobile map toggle */}
                <Button variant="ghost" size="sm" className="lg:hidden h-9 rounded-full" onClick={() => setShowMap(!showMap)}>
            ...
            {/* Category chips row */}
            <div className="backdrop-blur-xl bg-background/40 border-b border-border/20">
                  ...
                  {CATEGORIES.map((cat) => (
                    <motion.button
                      ...
                      className={`whitespace-nowrap px-3.5 py-1.5 rounded-full text-xs font-medium flex items-center gap-1.5 transition-all border ${
                        category === cat.value
                          ? 'bg-gradient-to-r from-[hsl(14,100%,57%)] to-[hsl(40,100%,49%)] text-white border-transparent shadow-lg shadow-[hsl(14,100%,57%)]/20'
                          : 'bg-background/50 backdrop-blur border-border/30 text-muted-foreground hover:text-foreground hover:border-border/60'
                      }`}
                    >
          ...
          {/* ── Content Grid ── */}
          <div ref={resultsRef} className="max-w-6xl mx-auto px-4 py-6">
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
                ...
                {isLoading ? (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {Array.from({ length: 4 }).map((_, i) => (
                      <div key={i} className="h-72 rounded-2xl backdrop-blur-xl bg-muted/30 border border-border/20 animate-pulse" />
                    ))}
                  </div>
                ) : listings.length > 0 ? (
                  <>
                      ...
                      <AnimatePresence mode="popLayout">
                        {listings.map((listing, i) => (
                          <motion.div
                            ...
                            className="rounded-2xl backdrop-blur-sm bg-background/40 border border-border/20 shadow-sm hover:shadow-xl hover:border-border/40 transition-all"
                          >
                  ...
                  /* ── Empty State ── */
                  <div className="relative flex flex-col items-center justify-center py-24 px-6 rounded-3xl backdrop-blur-2xl bg-background/30 border border-border/20">
                    <div className="absolute inset-0 overflow-hidden pointer-events-none rounded-3xl">
                      <motion.div
                        animate={{ scale: [1, 1.3, 1], opacity: [0.15, 0.25, 0.15] }}
                        transition={{ repeat: Infinity, duration: 5, ease: 'easeInOut' }}
                        className="absolute top-1/4 left-1/4 w-48 h-48 rounded-full bg-[hsl(14,100%,57%)]/15 blur-[80px]"
                      />
                      <motion.div
                        animate={{ scale: [1, 1.2, 1], opacity: [0.1, 0.2, 0.1] }}
                        transition={{ repeat: Infinity, duration: 7, ease: 'easeInOut', delay: 1 }}
                        className="absolute bottom-1/4 right-1/4 w-56 h-56 rounded-full bg-[hsl(40,100%,49%)]/12 blur-[80px]"
                      />
                    </div>
                    <motion.div
                      animate={{ y: [0, -12, 0] }}
                    ...
                    >
                      <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-[hsl(14,100%,57%)]/20 to-[hsl(40,100%,49%)]/20 backdrop-blur flex items-center justify-center border border-white/10">
                        <Truck className="w-10 h-10 text-[hsl(14,100%,57%)]" />
                      </div>
                    </motion.div>
                    ...
                    <p className="text-muted-foreground text-center max-w-sm mb-6 text-sm">
                      Try expanding your search area, adjusting filters, or browse everything.
                    </p>
                    <div className="flex gap-3">
                      <Button variant="outline" onClick={clearFilters} className="rounded-full backdrop-blur border-border/30">
              ...
              <div className={`lg:col-span-2 ${showMap ? '' : 'hidden lg:block'}`}>
                <div className="sticky top-48 h-[calc(100vh-13rem)] rounded-2xl overflow-hidden border border-border/20 backdrop-blur-xl bg-background/20 shadow-lg">
                  <SearchResultsMap
        ...
        {/* ── RIGHT SIDEBAR ── */}
        {/* ════════════════════════════════════════ */}
        <aside className="hidden xl:flex flex-col w-[280px] shrink-0 sticky top-16 h-[calc(100vh-4rem)] z-20 border-l border-border/20">
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            ...
            <motion.div
              ...
              className="rounded-2xl p-4 backdrop-blur-2xl bg-gradient-to-br from-[hsl(14,100%,57%)]/10 to-[hsl(40,100%,49%)]/10 border border-white/10"
            >
            ...
            <motion.div
              ...
              className="rounded-2xl p-4 backdrop-blur-2xl bg-background/40 border border-border/20"
            >
              ...
              <Button asChild variant="outline" size="sm" className="w-full rounded-full border-border/30 text-xs">
                <Link to="/list">
            ...
            <motion.div
              ...
              className="rounded-2xl backdrop-blur-2xl bg-background/40 border border-border/20 overflow-hidden"
            >
              <button
              ...
              >
                <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-[hsl(14,100%,57%)]/15 to-[hsl(40,100%,49%)]/15 flex items-center justify-center shrink-0">
                  ...
                  >
                    <div className="px-4 pb-4 space-y-3">
                      {[
                        ...
                        >
                          <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-[hsl(14,100%,57%)]/10 to-[hsl(40,100%,49%)]/10 flex items-center justify-center shrink-0 mt-0.5">
                      ...
                      <Button asChild variant="outline" size="sm" className="w-full rounded-full text-xs mt-2 border-border/30">
