// One-off agent-triggered BROADCAST sender for the New Exit Plan blog campaign.
// No auth required (gated by dedup table — cannot send same address twice).
...
serve(async (req) => {
  ...
  try {
    ...
    console.error("send-blog-broadcast-once error:", e);
