  ...
  try {
    ...
    switch (event.type) {
      ...
      case "payment_intent.amount_capturable_updated": {
        // This fires when a PaymentIntent enters requires_capture state (authorization hold)
        // Important for embedded payment form flow (no checkout session)
        const paymentIntent = event.data.object as Stripe.PaymentIntent;
        logStep("Processing payment_intent.amount_capturable_updated", { 
          paymentIntentId: paymentIntent.id,
        ...
        const bookingId = paymentIntent.metadata?.booking_id;
        const isAuthorizationHold = paymentIntent.metadata?.authorization_hold === 'true';
        const depositAmount = paymentIntent.metadata?.deposit_amount ? Number(paymentIntent.metadata.deposit_amount) : 0;
        
        if (bookingId && paymentIntent.status === 'requires_capture') {
          // Authorization hold successful - update booking
          const { error: updateError } = await supabaseClient
            .from("booking_requests")
            .update({
          ...
          if (updateError) {
            logStep("ERROR: Failed to update booking with authorization hold", { error: updateError.message });
          } else {
            logStep("Booking updated with authorization hold", { bookingId, paymentIntentId: paymentIntent.id });

            // Fetch booking details for notifications
            const { data: bookingData } = await supabaseClient
              .from("booking_requests")
              .select("shopper_id, host_id, total_price, listings(title)")
              .eq("id", bookingId)
              .single();

            if (bookingData) {
              const listingTitle = (bookingData.listings as any)?.title || "your booking";
              ...
              await supabaseClient.from("notifications").insert({
                ...
                message: `Your payment of $${bookingData.total_price} for "${listingTitle}" has been authorized. The host will review your request.`,
                link: "/dashboard",
              });
              ...
              await supabaseClient.from("notifications").insert({
                ...
                message: `A renter has submitted a request for "${listingTitle}". Their payment is authorized and awaiting your approval.`,
                link: "/dashboard",
              });

              logStep("Notifications created for authorization hold", { 
                shopperId: bookingData.shopper_id, 
