  ...
  <td valign="top" width="50%" style="padding:8px;">
    <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="background:#fff;">
      ...
      <tr><td style="padding-top:12px;">
        <div style="font-family:'Playfair Display',Georgia,serif;font-size:18px;font-weight:600;color:${COLORS.textDark};line-height:1.25;max-height:46px;overflow:hidden;">
          <a href="${esc(c.url)}${SOURCE}" style="color:${COLORS.textDark};text-decoration:none;">${esc(c.title)}</a>
        </div>
        <div style="font-family:'DM Sans',Arial,sans-serif;font-size:12px;color:${COLORS.textMuted};margin-top:6px;">📍 ${esc(c.location)}</div>
        <div style="font-family:'DM Sans',Arial,sans-serif;font-size:18px;color:${COLORS.orange};font-weight:700;margin-top:8px;">${esc(c.price)}</div>
        <div style="font-family:'DM Sans',Arial,sans-serif;font-size:12px;color:${COLORS.textMuted};margin-top:4px;">${esc(c.detail)}</div>
        <div style="margin-top:10px;">
          <a href="${esc(c.url)}${SOURCE}" style="font-family:'DM Sans',Arial,sans-serif;font-size:13px;color:${COLORS.orange};font-weight:600;text-decoration:none;">View Listing →</a>
        </div>
      </td></tr>
    </table>
  </td>`;
...
function pillButton(label: string, url: string, opts: { ghost?: boolean; full?: boolean } = {}): string {
  ...
  return `<a href="${esc(url)}" style="display:inline-block;background:${bg};color:${color};border:${border};font-family:'DM Sans',Arial,sans-serif;font-weight:600;font-size:14px;padding:14px 32px;border-radius:999px;text-decoration:none;${opts.full ? "display:block;text-align:center;" : ""}">${esc(label)}</a>`;
}
...
function toolColumn(t: ToolHighlight): string {
  ...
  <td valign="top" width="33.33%" style="padding:12px 10px;text-align:center;">
    ...
    <div style="font-family:'DM Sans',Arial,sans-serif;font-size:14px;color:#fff;font-weight:700;margin-bottom:6px;">${esc(t.label)}</div>
    <div style="font-family:'DM Sans',Arial,sans-serif;font-size:12px;color:#bdbdbd;line-height:1.5;">${esc(t.description)}</div>
  </td>`;
}
...
function feedbackPill(label: string, rating: string, p: ReportPayload): string {
  ...
  return `<a href="${esc(url)}" style="display:inline-block;background:${COLORS.pill};color:${COLORS.textDark};border:1px solid ${COLORS.divider};font-family:'DM Sans',Arial,sans-serif;font-size:13px;font-weight:600;padding:10px 18px;border-radius:999px;text-decoration:none;margin:0 4px;">${label}</a>`;
}
...
<link href="https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,600;0,700;1,400;1,600&family=DM+Sans:wght@400;500;600;700&display=swap" rel="stylesheet">
<style>
...
<body style="margin:0;padding:0;background:${COLORS.bgWarm};font-family:'DM Sans',Arial,sans-serif;color:${COLORS.textDark};">
    ...
    <tr><td align="center" style="padding:0;">
      <table role="presentation" class="container" cellpadding="0" cellspacing="0" border="0" width="640" style="max-width:640px;width:100%;background:#fff;">
        ...
        <tr><td style="background:${COLORS.bgDark};padding:36px 24px 28px;text-align:center;">
          ...
          <div style="font-family:'DM Sans',Arial,sans-serif;font-size:11px;letter-spacing:3px;color:#9a9a9a;text-transform:uppercase;">THE VENDIBOOK REPORT</div>
          <div style="font-family:'DM Sans',Arial,sans-serif;font-size:11px;color:#6a6a6a;margin-top:8px;">${esc(p.dateLabel)} · Issue No. ${p.issueNumber}</div>
        </td></tr>

        <!-- HERO HEADLINE -->
        <tr><td style="padding:48px 32px 36px;text-align:center;background:#fff;">
          <h1 style="font-family:'Playfair Display',Georgia,serif;font-size:34px;line-height:1.15;color:${COLORS.textDark};font-weight:700;margin:0;">${esc(p.heroHeadline)}</h1>
          <p style="font-family:'DM Sans',Arial,sans-serif;font-size:15px;color:${COLORS.textMuted};margin:18px 0 0;">Browse this week's freshest listings below.</p>
        </td></tr>
        ...
        <tr><td style="background:#fff;padding:40px 32px;">
          <div style="text-align:center;font-family:'DM Sans',Arial,sans-serif;font-size:11px;letter-spacing:3px;color:${COLORS.textMuted};text-transform:uppercase;padding-bottom:8px;">REFER & EARN</div>
          ...
          <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%">
            <tr>
              ...
              <td valign="top" class="stack">
                <h3 style="font-family:'Playfair Display',Georgia,serif;font-size:22px;color:${COLORS.textDark};font-weight:600;margin:0 0 10px;">${esc(ref.headline)}</h3>
                <p style="font-family:'DM Sans',Arial,sans-serif;font-size:14px;color:${COLORS.textMuted};line-height:1.55;margin:0 0 10px;">${esc(ref.body)}</p>
                <p style="font-family:'DM Sans',Arial,sans-serif;font-size:13px;color:${COLORS.textMuted};line-height:1.55;margin:0 0 14px;">${esc(ref.secondary)}</p>
                <a href="${esc(p.baseUrl)}/referral${SOURCE}" style="font-family:'DM Sans',Arial,sans-serif;font-size:14px;color:${COLORS.orange};font-weight:600;text-decoration:none;">Get your referral link →</a>
              </td>
            </tr>
          </table>
        </td></tr>
        ...
        <tr><td style="background:#fff;padding:48px 36px;">
          <div style="text-align:center;font-family:'DM Sans',Arial,sans-serif;font-size:11px;letter-spacing:3px;color:${COLORS.textMuted};text-transform:uppercase;padding-bottom:8px;">FROM THE VENDIBOOK TEAM</div>
          <div style="height:1px;background:${COLORS.divider};margin:0 auto 28px;width:60px;"></div>
          ${p.insightTitle ? `<h3 style="font-family:'Playfair Display',Georgia,serif;font-size:22px;color:${COLORS.textDark};font-weight:600;text-align:center;margin:0 0 18px;">${esc(p.insightTitle)}</h3>` : ""}
          <blockquote style="font-family:'Playfair Display',Georgia,serif;font-style:italic;font-size:22px;line-height:1.4;color:${COLORS.textDark};border-left:3px solid ${COLORS.orange};padding:8px 0 8px 20px;margin:0 0 24px;">"${esc(p.insightPullQuote)}"</blockquote>
          <p style="font-family:'DM Sans',Arial,sans-serif;font-size:15px;line-height:1.7;color:${COLORS.textDark};margin:0 0 16px;">${esc(p.insightBody)}</p>
          <p style="font-family:'Playfair Display',Georgia,serif;font-style:italic;font-size:14px;color:${COLORS.textMuted};margin:0;">— The Vendibook Team</p>
        </td></tr>

        <!-- TALK TO SOMEONE -->
        <tr><td style="background:#fff;padding:8px 32px 48px;text-align:center;">
          <div style="font-family:'DM Sans',Arial,sans-serif;font-size:11px;letter-spacing:3px;color:${COLORS.textMuted};text-transform:uppercase;padding:24px 0 8px;">WE'RE HERE</div>
          <div style="height:1px;background:${COLORS.divider};margin:0 auto 24px;width:60px;"></div>
          <h3 style="font-family:'Playfair Display',Georgia,serif;font-size:24px;color:${COLORS.textDark};font-weight:600;margin:0 0 14px;">Have questions about buying, selling, or renting?</h3>
          <p style="font-family:'DM Sans',Arial,sans-serif;font-size:14px;color:${COLORS.textMuted};line-height:1.6;margin:0 0 28px;max-width:480px;display:inline-block;">Whether you're exploring for the first time or ready to make a move — our team is available. No pressure, no scripts. Just a real conversation.</p>
          ...
          <p style="font-family:'DM Sans',Arial,sans-serif;font-size:12px;color:${COLORS.textMuted};margin:18px 0 0;">Average response time: under 2 hours</p>
        </td></tr>

        <!-- FEEDBACK -->
        <tr><td style="background:${COLORS.bgWarm};padding:32px 24px;text-align:center;">
          <div style="font-family:'DM Sans',Arial,sans-serif;font-size:11px;letter-spacing:3px;color:${COLORS.textMuted};text-transform:uppercase;padding-bottom:14px;">HOW WAS THIS EDITION?</div>
          <div>
            ${feedbackPill("👍 Helpful", "helpful", p)}
            ${feedbackPill("😐 It was okay", "okay", p)}
            ${feedbackPill("👎 Not for me", "not_for_me", p)}
          </div>
          <p style="font-family:'DM Sans',Arial,sans-serif;font-size:11px;color:${COLORS.textMuted};margin:18px 0 0;">Your feedback helps us make The Vendibook Report better.</p>
        </td></tr>
        ...
        <tr><td style="background:#fff;padding:36px 32px;text-align:center;border-top:1px solid ${COLORS.divider};">
          ...
          <p style="font-family:'DM Sans',Arial,sans-serif;font-size:12px;color:${COLORS.textMuted};line-height:1.6;margin:0 0 14px;">The Vendibook Report is sent twice weekly to all registered Vendibook users.</p>
          <p style="font-family:'DM Sans',Arial,sans-serif;font-size:11px;color:${COLORS.textMuted};margin:0 0 14px;">
          ...
          <p style="font-family:'DM Sans',Arial,sans-serif;font-size:11px;color:${COLORS.textMuted};margin:0 0 6px;">${esc(p.mailingAddress)}</p>
          <p style="font-family:'DM Sans',Arial,sans-serif;font-size:11px;color:${COLORS.textMuted};margin:0;">© ${new Date().getFullYear()} Vendibook. All rights reserved.</p>
        </td></tr>
...
export const DEFAULT_TOOLS: ToolHighlight[] = [
  ...
  { label: "Background-Checked Hosts", description: "Rent with confidence.", icon: "M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" },
];
