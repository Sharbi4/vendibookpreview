...
AS $$
  WITH listing_seller AS (
    ...
    SELECT ls.*,
      date_bin('15 minutes', greatest(_from, now()+make_interval(mins=>ls.minimum_notice_minutes)), 'epoch'::timestamptz) + interval '15 minutes' AS series_from,
      least(_to, _from + interval '31 days') AS series_to
    FROM listing_seller ls
  ), candidates AS (
    SELECT gs AS starts_at, gs + make_interval(mins=>b.default_duration_minutes) AS ends_at, b.*
    FROM bounds b
    CROSS JOIN LATERAL generate_series(b.series_from, b.series_to, interval '15 minutes') gs
  )
  ...
  WHERE EXISTS (SELECT 1 FROM public.seller_video_availability a WHERE a.seller_id=c.seller_id AND a.active AND a.weekday=extract(dow FROM c.starts_at AT TIME ZONE c.timezone)::smallint AND (c.starts_at AT TIME ZONE c.timezone)::time>=a.start_local_time AND (c.ends_at AT TIME ZONE c.timezone)::time<=a.end_local_time)
    AND NOT EXISTS (SELECT 1 FROM public.seller_video_blackouts b2 WHERE b2.seller_id=c.seller_id AND tstzrange(b2.starts_at,b2.ends_at,'[)') && tstzrange(c.starts_at,c.ends_at,'[)'))
