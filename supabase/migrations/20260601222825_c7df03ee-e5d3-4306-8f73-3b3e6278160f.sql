...
BEGIN
  ...
  THEN
    ...
    -- Look up host email + first name for transactional email
    ...
    -- Send branded activation email with expiration date
    IF v_host_email IS NOT NULL THEN
      PERFORM net.http_post(
        ...
        body := jsonb_build_object(
          ...
          'idempotencyKey', 'featured-activated-' || NEW.id::text,
    ...
    -- Optional push notification (only if user opted in)
