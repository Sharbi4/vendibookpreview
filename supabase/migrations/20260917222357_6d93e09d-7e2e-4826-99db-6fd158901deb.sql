REVOKE ALL ON FUNCTION public.create_video_walkthrough(uuid,timestamptz,uuid,text[],text) FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public.reschedule_video_walkthrough(uuid,timestamptz) FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public.cancel_video_walkthrough(uuid) FROM PUBLIC, anon;
CREATE POLICY video_walkthrough_provider_rooms_server_only ON public.video_walkthrough_provider_rooms FOR ALL TO service_role USING (true) WITH CHECK (true);
...
AS $$
  ...
  ORDER BY c.starts_at LIMIT 200
$$;
REVOKE ALL ON FUNCTION public.get_video_walkthrough_slots(uuid,timestamptz,timestamptz) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.get_video_walkthrough_slots(uuid,timestamptz,timestamptz) TO authenticated;