...
BEGIN
  ...
  -- IMPORTANT: document_type is an enum; cast to text before COALESCE
  doc_label := COALESCE(NEW.document_type::text, 'document');

  IF NEW.status = 'approved' THEN
    notification_title := 'Document Approved ✅';
    notification_message := 'Your document "' || doc_label || '" for "' || COALESCE(listing_title, 'your booking') || '" has been approved';
  ELSE
    notification_title := 'Document Rejected ❌';
    notification_message := 'Your document "' || doc_label || '" for "' || COALESCE(listing_title, 'your booking') || '" was rejected. Please upload a new one.';
  END IF;
