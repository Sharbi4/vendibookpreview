...
ALTER TABLE public.profiles
  ADD COLUMN IF NOT EXISTS grandfathered_listings boolean NOT NULL DEFAULT false;

-- Backfill: every account that exists right now is grandfathered.
UPDATE public.profiles SET grandfathered_listings = true WHERE grandfathered_listings = false;

-- Belt & braces: any user that already owns a listing is grandfathered
-- (covers auth users that somehow lack a profile row and get one later).
INSERT INTO public.profiles (id, grandfathered_listings)
SELECT DISTINCT l.host_id, true
FROM public.listings l
WHERE l.host_id IS NOT NULL
  AND NOT EXISTS (SELECT 1 FROM public.profiles p WHERE p.id = l.host_id)
ON CONFLICT (id) DO UPDATE SET grandfathered_listings = true;

-- 2) Server-side publish-limit enforcement
...
BEGIN
  ...
  ELSE
    v_becoming_published := (NEW.status = 'published' AND COALESCE(OLD.status, '') <> 'published');
  END IF;
...
DROP TRIGGER IF EXISTS trg_enforce_listing_publish_limit ON public.listings;
CREATE TRIGGER trg_enforce_listing_publish_limit
  BEFORE INSERT OR UPDATE OF status ON public.listings
  FOR EACH ROW
  EXECUTE FUNCTION public.enforce_listing_publish_limit();