...
CREATE TRIGGER on_auth_user_created_admin_notification
  ...
  FOR EACH ROW EXECUTE FUNCTION public.notify_admin_new_user_signup();
...
BEGIN
  ...
  PERFORM net.http_post(
    ...
    headers := jsonb_build_object(
      'Content-Type', 'application/json',
      'Authorization', 'Bearer eyJhbGciOiJIUzI1NiIsInJlZiI6Im5icmVoYndmc21lZGJlbHpudHFzIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjgxMDgzMTMsImV4cCI6MjA4MzY4NDMxM30.EkA-lGUmkLQ9rPAO-unLxGGGHVmPDdVR8awlA2ShVpU',
      'apikey', 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJIUzI1NiIsInJlZiI6Im5icmVoYndmc21lZGJlbHpudHFzIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjgxMDgzMTMsImV4cCI6MjA4MzY4NDMxM30.EkA-lGUmkLQ9rPAO-unLxGGGHVmPDdVR8awlA2ShVpU'
    ),
    body := jsonb_build_object(
      'type', 'new_listing',
...
DROP TRIGGER IF EXISTS on_listing_published_admin_notification ON public.listings;
CREATE TRIGGER on_listing_published_admin_notification
  AFTER INSERT OR UPDATE OF status ON public.listings
  FOR EACH ROW EXECUTE FUNCTION public.notify_admin_listing_published();